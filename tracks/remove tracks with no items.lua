...
if tracks ~= 0 then
  ...
  for i = 1, #t do
    if t[i][1] == max_dep and t[i][2] == '' then
      reaper.SetTrackSelected(reaper.GetTrack(0, i-1), true)
    end
  end
          ...
          if t[cur_tr_number+1] ~= nil then
            if t[cur_tr_number+1][1] > cur_tr_dep then -- if cur track is folder
              if t[cur_tr_number][2] == '' then -- is this track empty?
                ...
                if sel == 1 then -- all tracks in folder are empty so we can remove them
  ...
  reaper.PreventUIRefresh(-111)
  reaper.Undo_EndBlock('remove tracks with no items', 1)
