...
if tracks ~= 0 then
          ...
          if t[cur_tr_number+1] ~= nil then
            if t[cur_tr_number+1][1] > cur_tr_dep then -- if cur track is folder
              if t[cur_tr_number][2] == '' then -- is this track empty?
                if reaper.GetTrackNumSends(reaper.GetTrack(0, cur_tr_number-1), -1) == 0 then -- if cur track hasn't sends
                  sel = 1
  ...
  reaper.PreventUIRefresh(-111)
  reaper.Undo_EndBlock('select tracks with no items (except tracks with sends)', 1)
