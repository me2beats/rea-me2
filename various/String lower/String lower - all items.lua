...
function Utf8ToAnsi(s)
  ...
  for i = 1, s and s:len() or 0 do
    b = s:byte(i)
    if b < 128 then
      if nmdc[b] then r = r..nmdc[b]
      else r = r..string.char(b) end
    elseif a == 2 then
      a, j = a - 1, b
    elseif a == 1 then
...
if items > 0 then
  ...
  for i = 0, items-1 do
    ...
    for k = 0, takes-1 do
      tk =  reaper.GetTake(it, k)
      _, tk_name = reaper.GetSetMediaItemTakeInfo_String(tk, 'P_NAME', '', 0)
      tk_name_ansi = Utf8ToAnsi(tk_name)
      if tk_name_ansi then
        tk_name_ansi_lower = tk_name_ansi:lower()
        tk_name_utf_lower = AnsiToUtf8(tk_name_ansi_lower)
        reaper.GetSetMediaItemTakeInfo_String(tk, 'P_NAME', tk_name_utf_lower, 1)
      end
    end
  end
  reaper.PreventUIRefresh(-111); reaper.Undo_EndBlock('String lower - selected tracks', -1)
