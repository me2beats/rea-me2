...
--    if filename:find("[\128-\255]") == nil then
    len = #filename
    ...
    if samplomate_file ~= nil then
      ...
      reaper.MB('Samlomate file could not be found. Please run Samplomate preparing.lua','File Not Found',0)
    end
  else
    reaper.MB('Selected item source is MIDI. Please select audio item','Select audio',0)
  end
else
